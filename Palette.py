...
class palettedImage(object):
    def __init__(self, imgSource, imgPath='test.jpg', clusters=8, colorOffset=3, regime=0):
        ...
        self.outImgPath = imgPath.replace(imgPath.split(
            "/")[-1], "paletted_"+imgPath.split("/")[-1])
    ...
    def palettize(self):
        ...
        for i in range(0, len(np.unique(clt.labels_))):
            val = sorted(pinformed[np.where(clt.labels_ == i)], key=lambda x: spatial.distance.euclidean(
                x[0], self.centroids[i]))[0]
            # print(self.centroids[i], val[2] // 400, val[2] % 400)
            # print(val)
            # print(val[2] // 400, val[2] % 400)
            self.centers.append([int(val[2] % 400 / image_copy.shape[1] * image.shape[1]),
                                 int(val[2] // 400 / image_copy.shape[0] * image.shape[0])])

        outData = sorted(
            list(zip(self.centroids, self.hist, self.centers)), key=lambda x: x[1])
        #print(self.outData)
        self.outData = []
        ...
        if regime == 0:
            ...
            self.bar = self.plot_colors_rel(
                hist, clt.cluster_centers_, self.colorOffset)
        elif regime == 1:
            print("Using absolute palette colors scaling")
            self.bar = self.plot_colors_abs(
                hist, clt.cluster_centers_, self.colorOffset, self.clusters, 5)
        else:
    ...
    def draw_points(self):
        ...
        for i in range(len(data)):
            ...
            draw.ellipse((x-r, y-r, x+r, y+r),
                         (data[i]['r'], data[i]['g'], data[i]['b']), outline='white', width=5)
        image.save("test.jpg")
